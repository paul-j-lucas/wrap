...
**      src/doxygen.c
**
**      Copyright (C) 2018-2019  Paul J. Lucas
**
...
static dox_cmd_t const DOX_COMMANDS[] = {
  ...
  { "if",               DOX_INIT_EOL,     NULL },
  { "ifnot",            DOX_INIT_EOL,     NULL },
  ...
  { "link",             DOX_INIT_INLINE,  NULL },

  { "mainpage",         DOX_INIT_EOL,     NULL },
  { "manonly",          DOX_INIT_PRE,     "manonly" },
...
bool dox_parse_cmd_name( char const *s, char *dox_cmd_name ) {
  ...
  if ( s[0] == '@' || s[0] == '\\' ) {
    ...
    strncpy( dox_cmd_name, s + 1, dox_cmd_name_len );
    dox_cmd_name[ dox_cmd_name_len ] = '\0';
    return true;
  }
  return false;
}
